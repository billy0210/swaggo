...
var regexAttributes = map[string]*regexp.Regexp{
	"example": regexp.MustCompile(`(?i)\s+example\(.*\)`),
	// for Enums(A, B)
...
func (operation *Operation) parseAndExtractionParamAttribute(commentLine, objectType, schemaType string, param *spec.Parameter) error {
	...
	for attrKey, re := range regexAttributes {
		...
		case "example":
			...
			param.ParamProps.Schema.Example = value
		case "enums":
		...
		case "minimum":
			...
			param.Minimum = &n
		//将attribute改为加入schema，不然会出错
		case "default":
			value, err := defineType(schemaType, attr)
			if err != nil {
				return nil
			}
			param.ParamProps.Schema.Default = value
		case "maxlength":
...
func setEnumParam(attr, schemaType string, param *spec.Parameter) error {
	for _, e := range strings.Split(attr, ",") {
		...
		param.ParamProps.Schema.Enum = append(param.Enum, value)
	}
	return nil
}
...
// defineType enum value define the type (object and array unsupported), 现已支持object
